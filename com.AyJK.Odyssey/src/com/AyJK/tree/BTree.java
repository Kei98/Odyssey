...
//
//		BTreeNode<T> root;  //every tree has at least a root node
//
//
//	// ---------------------------------------------------------
...
//
//			while(i < root.count && key.compareTo(root.key.getDatai(i)) > 0)//keep incrementing
//	                    							  //in node while key >
//					                    			  //current value.
//			{
//				i++;
//			}
//
//			if(i <= root.count && key.equals(root.key.getDatai(i)))//obviously if key is in node
...
//	//  --------------------------------------------------------
//
//		public void split(BTreeNode<T> x, int i, BTreeNode<T> y)
//		{
//			BTreeNode<T> z = new BTreeNode<T>(order,null);//gotta have extra node if we are
//		                					//to split.
//
...
//			{
//				z.key.setDatai(j, y.key.getDatai(j+1)); //copy end of y into front of z
//
//			}
...
//				{
//					z.child.setDatai(k, y.child.getDatai(k + order)); //reassing child of y
//				}
//			}
//
...
//
//				x.child.setDatai(j+1, x.child.getDatai(j));  //shift children of x
//
//			}
...
//	// ----------------------------------------------------------
//
//		public void nonfullInsert(BTreeNode<T> x, int key)
//		{
...
//		{
//			BTreeNode<T> r = t.root;//this method finds the node to be inserted as 
//					 //it goes through this starting at root node.
//			if(r.count == 2*order - 1)//if is full
//			{
//				BTreeNode<T> s = new BTreeNode<T>(order,null);//new node
//
//				t.root = s;    //\
//		    			       // \	
//				s.leaf = false;//  \
...
//	       {
//				       
//			BTreeNode<T> temp = new BTreeNode<T>(order,null);//temp Bnode
//				
